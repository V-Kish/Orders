...
class EmojiChatBuckup extends Base {
    ...
    get keyboardPaddingButton() {
        return navigator().keyboardHeight + this.chatInput.inputHeight;
    }
...
/*    async sendFiles(files, fileIndex: number = 0, triesToSent: number = 0) {
    ...
    get top() {
        ...
        return navigator().deviceHeight - navigator().keyboardHeight
            - this.chatInput.inputHeight - hp(60) - this.getModalWindowHeight()
            - hp(80)
            - mistake
    }
    ...
    updateTop() {
        ...
        try {
            if (this.emojiFieldVisible || this.keyboardVisible) {
                this.refOnViewKeyboard.setNativeProps({ style: { top: this.top } });
            }
            else {
                this.refOnViewKeyboard.setNativeProps({ style: {
                        top: navigator().deviceHeight - this.chatInput.inputHeight
                        - hp(60) - this.getModalWindowHeight()
                        - hp(80)
                    }
                });
            }
    ...
    resetTop() {
        ...
        try {//640  //618
            ...
            this.refOnViewKeyboard.setNativeProps({ style: {
                top: navigator().deviceHeight
                - this.chatInput.inputHeight
                - hp(60)  - this.getModalWindowHeight()
                - hp(80)
                }
            });
    ...
    get scrollViewHeight() {
        const mistake = this._mistakeHeight !== null ? (this._mistakeHeight>60 ? this._mistakeHeight/2 : 0) : 0
        return navigator().deviceHeight
    ...
    resetScrollViewHeight(){
        if (this._messageListRef !== null){
            try {
                this._messageListRef.setNativeProps({style:{
                        height: navigator().deviceHeight
                        - hp(60)
                        - this.chatInput.inputHeight
                        - hp(80)
                    }});
    ...
    keyboardDidShow() {
        console.log('keyboardDidShow', this.keyboardVisible);
        this.updateScrollViewHeight();
        this.emojiField.update(navigator().keyboardHeight);
        this.keyboardVisible = true;
        this.emojiFieldVisible = false;
    ...
    keyboardDidHide() {
        console.log('keyboardDidShow hide', this.keyboardVisible);
