...
class EmojiChat extends Base {
  ...
  constructor(model: emojiChatProps) {
    ...
    this.resendMessage = new ResendMessage({
      id: 'Chats_ResendMessage',
      onExit: this.onEditHide,
    });
  ...
  get scrollViewHeight() {
    const mistake = 0;
    return (
      navigator().deviceHeight -
      navigator().keyboardHeight -
      hp(60) -
      this.chatInput.height -
      hp(80) -
      hp(10) -
      mistake
    );
  ...
  updateScrollViewHeight() {
    if (this._messageListRef !== null) {
      try {
        this._messageListRef.setNativeProps({
  ...
  setEmoji() {
    try {
      ...
      let rightStr = this._chatInput.textBox.value.substring(
        end,
        this._chatInput.textBox.value.length,
      );
      let lengthOfEmoji = this._emojiField.emojiButton.length;
      let str = leftStr + this._emojiField.emojiButton + rightStr;
      this._chatInput.selection = {
        start: start + lengthOfEmoji,
        end: end + lengthOfEmoji,
      };
  ...
  onResend(message: Message) {
    store()
      .contactsItems.refresh()
      .then(() => {
        message.userName = store().contactsItems.storage.get(
          message.uFrom,
        ).name;
        this.resendMessage.message = message;
  ...
  resend(message: Message) {
    ...
    forwardMessage.ownerUserFrom =
      message.ownerUserFrom !== -1 ? message.ownerUserFrom : message.uFrom;
  ...
  setEditing() {
    const chat = store().chats.current;
    if (
      chat.items.yPosition === null ||
      chat.items.yPosition >= chat.items.yValue - 300
    ) {
  ...
  newMessage(fileIndex?: number) {
    ...
    if (this.keyboardFilePreview.files[fileIndex]) {
      ...
      newMessage.messageType = MessageHelper.getMessageType(
        newMessage.message.fileNmae,
      );
  ...
  fileMessageObj(fileIndex) {
    return {
      ...
      mimeType: MessageHelper.getFileExtension(
        this.keyboardFilePreview.files[fileIndex].name,
      ),
  ...
  async sendFile(newMessage) {
    try {
      ...
      const base64 = await MessageHelper.fileToBase64(
        newMessage.message.fileHash,
      );
      const body = {
        fileBase64: base64,
  ...
  async uploadFileToServer(newMessage) {
    ...
    let file = this._fileMessage.find(
      (f) => f.name === newMessage.message.fileNmae,
    );
    ...
    try {
      const response = await fetch(
        `${AppSettings.chatEndpoint}/${currentUser().userToken}/${
          currentUser().currentOsbb.hash
        }/chat/${store().chats.selectedChatId}/file/add-as-filebase`,
        {
          method: 'POST',
          headers: {
  ...
  updateTop() {
    ...
    try {
      if (this.emojiFieldVisible || this.keyboardVisible) {
        this.refOnViewKeyboard.setNativeProps({
          style: {
            top:
              this.top +
              (Platform.OS === 'ios' ? currentUser().saveAreaInset.bottom : 0),
          },
        });
      } else {
        this.refOnViewKeyboard.setNativeProps({
