...
type chatInputProps = {
    ...
    onFocus?: (textBox: ChatInputBuckup) => void;
    onContentSizeChange: (height: number, delta: number) => void;
    selection?: { start: number, end: number };
}
class ChatInputBuckup extends Base {
    ...
    constructor(model: chatInputProps) {
        ...
        this._inputLines = 1;
        this._inputHeight = hp(60);
        this._textBoxHeight = hp(50);
    ...
    set refInputContainer(value: any) {
    ...
    onContentSizeChange(event: any) {
        ...
        const maxLines = 5
        const minHeight = hp(50)
        const maxHeight = this._initialInputHeight*maxLines
        if(height > minHeight && height < maxHeight){
            this._inputHeight = height+10
            this._textBoxHeight = height
            this.forceUpdate()
            this._model.onContentSizeChange(this._inputHeight, 0)
        } else if(height < minHeight) {
            this._inputHeight = hp(60)
            this._textBoxHeight = hp(50)
            this.forceUpdate()
            this._model.onContentSizeChange(this._inputHeight, 0)
        } else if(height > maxHeight && this._textBoxHeight<maxHeight){
            this._inputHeight = maxHeight+10
            this._textBoxHeight = maxHeight
            this.forceUpdate()
            this._model.onContentSizeChange(this._inputHeight, 0)
        }
        ...
        //     this._model.onContentSizeChange(height, 0);
        //     return;
        // }
    ...
    updateHeight() {
        ...
        //  console.log('updateHeight3', this.height)
        //  console.log('updateHeight3 value', this.textBox.value)

        // this._refInputContainer.setNativeProps({
        //     style: { height:this.height > 120 ? 120 :  this.height+hp(10) }
        // });
    }
    onFocus() {
        if(typeof this._model.onFocus === 'function'){
