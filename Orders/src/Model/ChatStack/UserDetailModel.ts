...
class UserDetailModel extends BaseModel {
  ...
  //     const addresses = this._userModel?.detailInfo.filter(
  //       (e) => e.osbbName === currentUser().currentOsbb?.name,
  //     );
  //     let fullAddress = '';
  //     addresses.forEach((detail) => {
  //       fullAddress +=
  //         this.getAddressFromDetail(detail) +
  //         (addresses.length > 1 ? '\r\n' : '');
  //     });
  //     return fullAddress;
  //   } else {
  ...
  get chatList() {
    return store()
      .chats.storage.filter((i) => i.isPublic)
      .filter(
        (c) => c.members && c.members.find((f) => f.id == 56) !== undefined,
      );
  }
}
